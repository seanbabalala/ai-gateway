...
export function ThemeToggle() {
  ...
  return (
    <div
      ...
      className="relative flex items-center rounded-lg bg-[var(--background-secondary)] p-0.5 shadow-[0_1px_2px_rgba(5,46,36,0.05)]"
    >
      {modes.map(({ mode: m, icon: Icon, labelKey }) => {
        ...
        <button
          ...
          title={label}
          className={cn(
            'relative z-10 flex items-center justify-center rounded-lg p-1.5 transition-colors duration-200 cursor-pointer',
            mode === m
              ? 'text-white'
