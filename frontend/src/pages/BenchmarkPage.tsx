...
function formatSource(source: string, t: TFunction) {
  return t(`benchmark.source.${source}`, { defaultValue: source.replaceAll('_', ' ') })
}
...
function MetricStrip({ metrics }: { metrics: BenchmarkMetrics }) {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-5">
      ...
      <div>
        <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--foreground-dim)]">
          {t('benchmark.latency.p99')}
        </div>
        <div className="mt-1 font-mono text-[15px] font-bold text-[var(--foreground)]">
          {formatLatency(metrics.latency_ms.p99_ms)}
        </div>
      </div>
      <div>
        <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--foreground-dim)]">
...
export function BenchmarkPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useBenchmarkReport({
    ...
    limit: 5000,
    scope: apiKeyFilter ? { id: apiKeyFilter } : undefined,
  })
  ...
  const modelOptions = useMemo(() => {
    ...
    for (const node of nodesData?.nodes || []) {
      ...
      for (const model of node.realtime?.models || []) models.add(model)
    }
    return [
  ...
  const maxSourceCalls = Math.max(...data.by_source_format.map((item) => item.calls), 0)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          subtitle={t('benchmark.metrics.failed', { count: data.summary.failed })}
          icon={MonitorCheck}
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={data.summary.throughput_rpm.toFixed(2)}
      ...
      <div className="grid gap-5 xl:grid-cols-[1.5fr_1fr]">
        ...
        <div className="space-y-5">
          <CardStatic>
            <CardHeader>
              <CardTitle>{t('benchmark.bySource.title')}</CardTitle>
            </CardHeader>
            <CardContent>
              {data.by_source_format.length === 0 ? (
                <EmptyState
                  icon={BarChart3}
                  title={t('benchmark.empty.noSourceTitle')}
                  description={t('benchmark.empty.noSourceDescription')}
                  className="py-6"
                />
              ) : (
                <div className="space-y-4">
                  {data.by_source_format.map((item) => (
                    <div key={item.source_format}>
                      <div className="mb-1.5 flex items-center justify-between gap-2">
                        <span className="truncate text-[12px] font-semibold text-[var(--foreground)]">
                          {formatSource(item.source_format, t)}
                        </span>
        ...
        <CardStatic>
          ...
          <CardContent>
            ...
            {data.top_errors.length > 0 && (
              <div className="mt-5">
                <div className="mb-2 text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--foreground-dim)]">
                  {t('benchmark.errors.title')}
                </div>
                <div className="space-y-2">
                  {data.top_errors.map((item) => (
                    <div key={item.error} className="flex items-start justify-between gap-3 rounded-lg bg-red-500/8 px-3 py-2">
