...
v1.7 adds model enrichment metadata to model rows and fresh-default metadata to provider rows. Model rows can now include `enrichment.lifecycle`, `enrichment.specs`, `enrichment.benchmarks`, and enrichment source metadata. Provider rows can now include `recommended_model_buckets`, `latest_model_hints`, `recommended_models`, and `enrichment_summary`. These fields let the Dashboard prefer fresher stable models with usable price metadata for Add Node defaults while keeping the complete model list available for manual selection.
