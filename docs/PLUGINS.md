...
The manager supports exact versions, `^`, `~`, wildcard `*`, and simple comparator ranges such as `>=0.3.0 <0.4.0`. Missing compatibility metadata is allowed with a warning; incompatible ranges fail before the declaration is written.
