...
import { RouteTarget, TierConfig } from '../config/gateway.config';
import {
...
export class RoutingRecommendationService {
  ...
  private recommendForTier(
    ...
    const route = this.normalizeRecommendationRoute(tierConfig);
    if (!route) {
      return this.unconfiguredTierRecommendation(tier);
    }
    ...
    const candidates = this.uniqueTargets(route.fallbacks).filter(
      (target) => this.targetKey(target) !== this.targetKey(route.primary),
    );
  ...
  private normalizeRecommendationRoute(route: TierConfig): RecommendationRoute | null {
    const splitTargets = (route.split || []).map((variant) => this.toRouteTarget(variant));
    if (splitTargets.length > 0) {
      return {
        primary: splitTargets[0],
        fallbacks: this.uniqueTargets([
          ...splitTargets.slice(1),
          ...(route.fallbacks || []),
        ]),
      };
    }

    const weightedTargets = (route.targets || []).map((target) => this.toRouteTarget(target));
    if (weightedTargets.length > 0) {
      return {
        primary: weightedTargets[0],
        fallbacks: this.uniqueTargets([
          ...weightedTargets.slice(1),
          ...(route.fallbacks || []),
        ]),
      };
    }
  ...
  private toRouteTarget(target: Pick<RouteTarget, 'node' | 'model'>): RouteTarget {
    return { node: target.node, model: target.model };
  }

  private unconfiguredTierRecommendation(tier: string): AdaptiveRoutingRecommendation {
    return {
      id: `${tier}:unconfigured`,
      tier,
      type: 'collect_more_data',
      current_primary: { node: 'unconfigured', model: 'unconfigured' },
      suggested_primary: null,
      suggested_fallbacks: [],
      reasons: ['Tier has no primary or targets configured, so no routing recommendation can be generated.'],
      confidence: 0,
      potential_savings: {
        cost_usd_per_1k_calls: 0,
        window_cost_usd: 0,
      ...
      risks: ['Fix the routing tier configuration before using recommendation evidence.'],
      evidence: {
        current: null,
        candidate: null,
        tier_calls: 0,
