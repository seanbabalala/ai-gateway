...
function validateTier(
  ...
  const hasTargets = tierValue.targets !== undefined;
  const hasSplit = tierValue.split !== undefined;
  const requiresLegacyRoute = !hasTargets && !hasSplit;

  if (tierValue.primary !== undefined || requiresLegacyRoute) {
    validateRouteTargetShape(tierValue.primary, `${tierPath}.primary`, issues);
  }

  if (tierValue.fallbacks === undefined && !requiresLegacyRoute) {
    // targets and split are self-contained routing modes; legacy fallbacks are optional.
  } else if (!Array.isArray(tierValue.fallbacks)) {
    issues.push(
      issue(
        'error',
        'missing_required_field',
